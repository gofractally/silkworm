...
class Execution final : public Stage {
  ...
  private:
    ...
    static void unwind_state_from_changeset(datastore::kvdb::ROCursor& source_changeset, datastore::kvdb::RWCursor& plain_state_table,
                                            datastore::kvdb::RWCursor& plain_code_table, BlockNum unwind_to);

    //! \brief Revert State for given address/storage location
    static void revert_state(ByteView key, ByteView value, datastore::kvdb::RWCursor& plain_state_table,
